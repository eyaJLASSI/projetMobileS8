...
class PosteService {
    ...
    //Avoir tous les postes
    
    private func getAllPostesByIdUrl(idF : Int) -> String
    {
        "postes/\(idF)"
    }
    
    
    public func getAllPostes(idF: Int) async -> Result<[PosteDTO], Error> {
        // Concatener l'host avec l'uri
        guard let getAllPostesURL = URL(string: "\(api)\(getAllPostesByIdUrl(idF: idF))") else {
            return .failure(PosteErrors.failedUrl)
        }
        
        // Créer une requête Get
        var request = URLRequest(url: getAllPostesURL)
        request.httpMethod = "GET"
        ...
        do {
            ...
            if httpresponse.statusCode == 200 {
                guard let decoded: [PosteDTO] = await JSONHelper.decodeOne(data: data) else {
                    return .failure(PosteErrors.failedDecode)
                }
